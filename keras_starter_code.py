...
split = int(train.shape[0]/2)

# train model, test on 15% hold out data
# model.fit(train, label_log, batch_size=32, nb_epoch=30, verbose=2, validation_split=0.15)
model.fit(train[:split], label_log[:split], batch_size=32, nb_epoch=30, verbose=2, validation_split=0.15)

# generate prediction file
# preds = np.expm1(model.predict(test, verbose=0).flatten())
preds = np.expm1(model.predict(train[split:], verbose=0).flatten())

error = rmsle.error(preds, np.expm1(label_log[split:]))
print "Error:", error
